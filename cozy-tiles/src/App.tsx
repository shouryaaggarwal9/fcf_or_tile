...
import { advanceSession, BOOSTERS, BOOSTER_ORDER, pick, price, purchase, restartSession, unavailable, WIN_COINS } from "./session";
...
function TileIcon({ kind }: { kind: TileKind }) {
  return (
    <svg viewBox="0 0 64 64" fill="none" aria-hidden="true" focusable="false">
      {EXTRA_SYMBOLS[kind] && (
        <path d={EXTRA_SYMBOLS[kind].path} fill={EXTRA_SYMBOLS[kind].color}
          stroke="#35465b" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      )}
      {kind === "sun" && (
...
export default function App() {
  ...
  const [layout, setLayout] = useState(() => boardBounds(loaded.session.game.board));
  ...
  function commit(next: Session) {
    ...
    setSaveWarning(saved ? "" : "Progress could not be saved. Keep this page open; browser storage may be full or disabled.");
    setSession(next);
  ...
  function request(action: Booster) {
    if (inputLocked.current) return;
    setNotice("");
    if (action === "slot" && !unavailable(session, action) && price(session, action) === 0) {
  ...
  function apply(action: Booster) {
    ...
    if (action === "slot") feedback.slot(); else feedback.booster();
  }
    ...
    <main className="game-shell">
      <header className="topbar">
        <div>
          ...
          {difficulty.gentle && <span className="level-note">A gentle puzzle</span>}
        </div>
        ...
        <div className="topbar-controls">
          ...
          <button className="quiet-button" onClick={() => setShowSettings(true)}
            aria-haspopup="dialog" disabled={busy}>Settings</button>
        </div>
      </header>
      ...
      <section className="play-area" aria-label="Tile board">
        ...
        <div className="board" style={{ aspectRatio: `${layout.width} / ${layout.height}`, maxWidth: `${layout.width * 88}px` }}>
          {game.board.map((tile) => {
      ...
      <footer className="game-footer">
        <div className="booster-bar">
          {capacity === 6 && (
            <button className="booster-button" onClick={() => request("slot")}
              disabled={busy || Boolean(unavailable(session, "slot"))}>
              + Slot <span className="booster-price">{price(session, "slot") || "Free"}</span>
            </button>
          )}
          {BOOSTER_ORDER.map((action) => (
            <button key={action} className="booster-button"
              onClick={() => request(action)}
              disabled={busy || Boolean(unavailable(session, action))}>
              {BOOSTERS[action].label}
              <span className="booster-price">{price(session, action) || "Free"}</span>
            </button>
          ))}
        ...
        <p role="status" aria-live="polite">{notice || statusText}</p>
      ...
      {lost && !pending && (
        <div className="result-overlay">
          <section className="result-card" aria-labelledby="result-title">
            ...
            <h2 id="result-title" ref={resultHeading} tabIndex={-1}>Out of slots</h2>
            <p>Try clearing a triple before collecting other symbols.</p>
            <div className="recovery-row">
              {capacity === 6 && !unavailable(session, "slot") && (
                <button className="primary-button" onClick={() => request("slot")}>
                  Open seventh slot
                </button>
              )}
              {canUndo && <button className="primary-button" onClick={() => request("undo")}>Undo last pick</button>}
              {!unavailable(session, "wand") && (
                <button className="primary-button" onClick={() => request("wand")}>Use wand</button>
              )}
              <button className="primary-button" onClick={restart}>Try again</button>
            </div>
          </section>
        </div>
      )}
      ...
      {pending && (
        <div className="result-overlay" role="dialog" aria-modal="true"
          ...
          <section className="result-card">
            ...
            <p className="confirm-description">{BOOSTERS[pending].description}</p>
            <p className="confirm-price">
              {session.settings.relaxed ? "Free in Relaxed Mode" : `Costs ${BOOSTERS[pending].price} coins · balance ${session.coins}`}
            </p>
            <div className="recovery-row">
              <button className="primary-button" onClick={() => apply(pending)}>
                {session.settings.relaxed ? "Activate" : `Spend ${BOOSTERS[pending].price}`}
              </button>
              <button className="quiet-button" onClick={() => setPending(null)}>Cancel</button>
            </div>
          </section>
        </div>
      )}
      ...
      {showSettings && (
        <div className="result-overlay" role="dialog" aria-modal="true"
          ...
          <section className="result-card">
            ...
            {([
              ["sound", "Sound effects"], ["vibration", "Vibration"], ["relaxed", "Relaxed Mode"],
            ] as const).map(([key, label]) => (
              <label key={key} className="setting-row">
                <input type="checkbox" checked={session.settings[key]}
                  onChange={(event) => {
                    const next = { ...session, settings: { ...session.settings, [key]: event.target.checked }, revision: session.revision + 1 };
                    commit(next);
                    feedback.configure(next.settings);
                  }} />
                <span>{label}</span>
            ...
            <p className="confirm-description">Relaxed Mode makes boosters and the extra slot free. Matching rules stay the same.</p>
      ...
      {game.status === "won" && (
        <div className="result-overlay">
          <section className="result-card" aria-labelledby="result-title">
            <div className="result-symbol" aria-hidden="true">✦</div>
            <h2 id="result-title" ref={resultHeading} tabIndex={-1}>Lovely matching!</h2>
            <p>You cleared every tile. +{session.rewardedThrough >= session.level ? WIN_COINS : 0} coins earned.</p>
